...
function seed() {
  ...
  tx();
  console.log(`[db] seeded: knowledge=${Object.keys(raw.knowledge).length}, rules=${raw.rules.length}, rulesets=${Object.keys(rulesets).length}, products=${Object.keys(products).filter((k) => !k.startsWith("_")).length}`);
}
